...
from bokeh.models import HoverTool, BoxZoomTool, WheelZoomTool, ResetTool, PanTool, BoxSelectTool, TapTool, ResizeTool
...
kmeans.fit([v for v in data_d2v])
...
for i in xrange(K):
  ...
  clust_depth[i]=-np.mean(sorted(temp_score,reverse=True)[:10])#]int(np.sqrt(len(topic[i])))])
...
sorted_words_all_translated = translate_client.translate(sorted_words_all[:10], source_language='vi', target_language='en')
...
df = pd.DataFrame(zip(word_d2v,[kmeans_label[w] for w in word_d2v],
                      metrics_clean),columns=['word','topic','metric'])

num_top_scoring_words = 500
kmeans_label_color = [kmeans_label_ranked[w]+1 for w in word_d2v]
top_scoring_words = [(w,d[0],d[1],k,m) for w,d,k,m in zip(word_d2v,X_2D,kmeans_label_color,metrics_clean) if w in score_words]
...
p = figure(plot_width=500, plot_height=500, title="Distributed Word Embeddings: " + str(K) + " Topics", 
           tools=['box_zoom', 'box_select', 'pan','wheel_zoom','reset', 'save','tap', 'resize'])
...
text = ['Hello, world!','goodby, cruel world']                        
translation = translate_client.translate(text, target_language=target)
print translation[0]['translatedText']
print translation[1]['translatedText']
